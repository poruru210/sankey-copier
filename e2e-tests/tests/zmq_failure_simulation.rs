...
async fn test_failed_send_moved_to_dead_letter() {
    ...
    let id = db.record_failed_send(topic, &payload, err, 5).await.expect("record failed send");
    assert!(id > 0);

    // Wait up to 15 seconds for the retry worker to move the item to dead-letter
    let mut found = false;
    for _ in 0..30 {
        let rows = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM failed_outgoing_dead_letters WHERE original_id = ?")
            .bind(id)
            .fetch_one(db.pool())
            .await
            .unwrap_or(0);
        ...
        let processed: i64 = sqlx::query_scalar("SELECT processed FROM failed_outgoing_messages WHERE id = ?")
            .bind(id)
            .fetch_one(db.pool())
            .await
            .unwrap_or(0);
    ...
    assert!(found, "failed item was not moved to dead-letter within timeout");
}
