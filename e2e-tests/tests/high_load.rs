...
use e2e_tests::helpers::{setup_test_scenario, default_test_slave_settings, start_eas_and_wait_for_connected};
...
async fn test_high_load_trade_routing() {
    ...
    let db = Database::new(&server.db_url()).await.expect("DB connect failed");
    ...
    setup_test_scenario(&db, master_account, &slave_refs, |_| default_test_slave_settings())
    .await
    .expect("Failed to setup DB scenario");

    // Create EA simulators
    let mut master = MasterEaSimulator::new(&server.zmq_pull_address(), &server.zmq_pub_address(), master_account).expect("failed to create master");
    ...
    for i in 0..total_signals {
        let signal = master.create_open_signal(i as i64, "EURUSD", "BUY", 0.1, 1.2345, None, None, i as i64);
    ...
    assert!(successes > (total_signals / 2), "Too many send failures under load: {} of {}", successes, total_signals);
    ...
    let pending = db.fetch_pending_failed_sends(100).await.expect("db fetch failed");
    assert!(pending.is_empty(), "Found unexpected pending failed sends after high load: {}", pending.len());
}
