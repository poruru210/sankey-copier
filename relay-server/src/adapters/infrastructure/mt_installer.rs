...
pub struct MtInstaller {
    ...
    /// シンボル検索候補リスト
    symbol_search_candidates: Vec<String>,
}

impl MtInstaller {
    /// 新しいインストーラーを作成
    pub fn new(components_base_path: PathBuf, symbol_search_candidates: Vec<String>) -> Self {
        Self {
            components_base_path,
            symbol_search_candidates,
        }
    }
    ...
    pub fn from_config(config: &crate::config::Config) -> Self {
        ...
        // Extract and flatten symbol candidates from config
        let mut candidates = Vec::new();
        for group in &config.symbol_mapping.synonym_groups {
            candidates.extend(group.clone());
        }
        // Remove duplicates and sort for deterministic output
        candidates.sort();
        candidates.dedup();

        Self::new(base_path, candidates)
    }
    ...
    fn install_config(&self, mql_path: &Path, resolved_ports: &ResolvedPorts) -> Result<()> {
        ...
        // Symbol Search Candidates
        let mut candidates_section = String::new();
        if !self.symbol_search_candidates.is_empty() {
            candidates_section = format!(
                "{le}[SymbolSearch]{le}Candidates={}{le}",
                self.symbol_search_candidates.join(",")
            );
        }
...
PublisherPort={}{le}\
{}",
            resolved_ports.receiver_port, resolved_ports.sender_port, candidates_section
        );
...
mod tests {
    ...
    fn test_installer_with_custom_path() {
        ...
        let installer = MtInstaller::new(custom_path.clone(), vec![]);
    ...
    fn test_install_dll_32bit() {
        ...
        // Install
        let installer = MtInstaller::new(temp_components.path().to_path_buf(), vec![]);
    ...
    fn test_install_config() {
        ...
        let installer = MtInstaller::new(
            temp_mt.path().to_path_buf(),
        ...
        // Verify Symbol Search Candidates
        assert!(content.contains("[SymbolSearch]"));
        assert!(content.contains("Candidates=GOLD,XAUUSD"));
    }
